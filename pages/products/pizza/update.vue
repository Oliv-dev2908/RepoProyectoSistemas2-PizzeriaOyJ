...
const tienePatronesRepetitivos = (texto) => {
  return patronesRepetitivos.some(p => p.test(texto));
};
...
const validateForm = () => {
  ...
  if (!nombreTrim) {
    ...
    valid = false;
  } else if (tienePatronesRepetitivos(nombreTrim)) {
  ...
  if (!descripcionTrim) {
    ...
    valid = false;
  } else if (tienePatronesRepetitivos(descripcionTrim)) {
